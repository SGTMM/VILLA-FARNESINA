...
function start() {
    ...
    slide = new THREE.Mesh(new THREE.PlaneGeometry(5.3, 3), new THREE.MeshBasicMaterial({ map: new THREE.TextureLoader().load("Screenshot (" + counter + ").png") }))
    slide.position.z = -2
    scene.add(slide)
    ...
    for (var i = 1079; i < 1110; i++) {
        textureArray[i] = new THREE.TextureLoader().load("Screenshot (" + i + ").png")
    }
...
function updateTexture() {
    ...
    if (slide) {
        ...
        slide.material = new THREE.MeshBasicMaterial({ map: temp_texture })
    }
}
